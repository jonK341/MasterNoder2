...
- [Development guidelines](#development-guidelines)
    - [General MasterNoder2 Core](#general-masternoder2-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MasterNoder2 Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play MN2s (tMN2)" on the test network, if you
...
MasterNoder2 Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/masternoder2-project/masternoder2/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_masternoder2
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_masternoder2 --log_level=test_suite
$ valgrind -v --leak-check=full src/masternoder2d -printtoconsole
```
...
# A coverage report will now be accessible at `./test_masternoder2.coverage/index.html`.
```
...
- BitcoinMiner : Generates MN2s (if wallet is enabled).
...
However, in open source software such as MasterNoder2 Core, where everyone uses
...
pay attention to for reviewers of MasterNoder2 Core code.

General MasterNoder2 Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MasterNoder2 Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against MasterNoder2 Core so that they can be integrated
...
file descriptors it will cause problems with MasterNoder2's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:MasterNoder2-Project/MasterNoder2.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `masternoder2-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `masternoder2-cli` and the GUI debug console use this table to determine how to
