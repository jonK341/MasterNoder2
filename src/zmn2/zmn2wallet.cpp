...
CzMN2Wallet::CzMN2Wallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zmn2 seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZMN2Seed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZMN2Seed_deprecated()) {
                    LogPrintf("%s: Updated zMN2 seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zmn2 seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zmn2 wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzMN2Wallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZMN2Count(nCountLastUsed);
    else if (!walletdb.ReadZMN2Count(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzMN2Wallet::Lock()
{
    seedMaster = 0;
}

void CzMN2Wallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzMN2Wallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZMN2(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzMN2Wallet::LoadMintPoolFromDB()
{
...
void CzMN2Wallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzMN2Wallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzMN2Wallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zmn2Tracker->HasPubcoinHash(pMint.first)) {
...
bool CzMN2Wallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZMN2(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zmn2Tracker which also adds to database
    pwalletMain->zmn2Tracker->Add(dMint, true);
...
void CzMN2Wallet::SeedToZMN2(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzMN2Wallet::GetZerocoinSeed(uint32_t n)
{
...
void CzMN2Wallet::GenerateDeterministicZMN2(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzMN2Wallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzMN2Wallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzMN2Wallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
