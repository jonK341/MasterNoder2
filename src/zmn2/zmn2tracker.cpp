...
CzMN2Tracker::CzMN2Tracker(std::string strWalletFile)
{
...
CzMN2Tracker::~CzMN2Tracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzMN2Tracker::Init()
{
...
bool CzMN2Tracker::Archive(CMintMeta& meta)
{
...
bool CzMN2Tracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzMN2Tracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzMN2Tracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzMN2Tracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzMN2Tracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzMN2Tracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzMN2Tracker::GetSerialHashes()
{
...
CAmount CzMN2Tracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzMN2Tracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzMN2Tracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzMN2Tracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzMN2Tracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzMN2Tracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzMN2Tracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzMN2Tracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzMN2Tracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzMN2Tracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzMN2Tracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzMN2Wallet* zMN2Wallet)
{
    bool iszMN2WalletInitialized = (NULL != zMN2Wallet);
    CMintMeta meta;
    ...
    if (! iszMN2WalletInitialized)
        zMN2Wallet = new CzMN2Wallet(strWalletFile);
    meta.isSeedCorrect = zMN2Wallet->CheckSeed(dMint);
    if (! iszMN2WalletInitialized)
        delete zMN2Wallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzMN2Tracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzMN2Tracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzMN2Tracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzMN2Tracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzMN2Tracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzMN2Tracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzMN2Wallet* zMN2Wallet = new CzMN2Wallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zMN2Wallet);
        }
        delete zMN2Wallet;
        LogPrint("zero", "%s: added %d dzmn2 from DB\n", __func__, listDeterministicDB.size());
    }
