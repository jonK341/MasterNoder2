...
// The zMN2 block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zMN2 is a hash of the serial
...
{
    //Create an output returning the zMN2 that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZMN2OutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zMN2 output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zMN2", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZMN2OutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zMN2 output", __func__);
...
{
    CzMN2Tracker* zmn2Tracker = pwallet->zmn2Tracker.get();
    CMintMeta meta;
    if (!zmn2Tracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zmn2Tracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a MN2 stake is the outpoint
