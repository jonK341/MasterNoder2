...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zmn2spends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 MN2 at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zMN2 send
enum ZerocoinSpendStatus {
    ZMN2_SPEND_OKAY = 0,                            // No error
    ZMN2_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZMN2_WALLET_LOCKED = 2,                         // Wallet was locked
    ZMN2_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZMN2_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZMN2_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZMN2_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZMN2_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZMN2_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZMN2_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZMN2_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZMN2_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZMN2_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZMN2_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZMN2_SPENT_USED_ZMN2 = 14,                      // Coin has already been spend
    ZMN2_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZMN2_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZMN2OutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzmn2Auto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzMN2Tracker> zmn2Tracker;
    ...
    void setZWallet(CzMN2Wallet* zwallet)
    {
        zwalletMain = zwallet;
        zmn2Tracker = std::unique_ptr<CzMN2Tracker>(new CzMN2Tracker(strWalletFile));
    }

    CzMN2Wallet* getZWallet() { return zwalletMain; }
    ...
    /** zMN2 reset */
    boost::signals2::signal<void()> NotifyzMN2Reset;
