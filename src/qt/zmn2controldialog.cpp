...
#include "ui_zmn2controldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zmn2Tracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zmn2Tracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zMN2.";
            else if (!mint.isSeedCorrect)
                strReason = "The zMN2 seed used to mint this zMN2 is not the same as currently hold in the wallet";
            else
